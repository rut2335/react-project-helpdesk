...
const Tickets = () => {
    ...
    const deleteTicket = async (ticketId: number, e: React.MouseEvent) => {
        e.stopPropagation();
        try {
            await Swal.fire({
                title: "מחיקת פניה",
                ...
                if (result.isConfirmed) {
                    ...
                    await Swal.fire("נמחק!", "", "success");
                }
            });
        } catch (error) {
            console.error("Error deleting ticket:", error);
            await Swal.fire("שגיאה", "  אירעה שגיאה במחיקת הפניה! אין אפשרות למחוק פניה עם תגובות.", "error");
        }
    }
            ...
            <Box sx={{ p: 4 }}>
                ...
                <Grid container spacing={4} alignItems="stretch">
                    {sortedTickets.map((ticket: TicketResponse) => (
                        <Grid  size={{ xs: 12, md: 6, lg: 4 }} key={ticket.id} sx={{ display: 'flex' }}>
                            <Card
                                elevation={0}
                                sx={{
                                    width: '100%',
                                        ...
                                        {(isAdmin || isAgent) && (
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                                                ...
                                                <Typography variant="caption" sx={{ color: '#546e7a', fontWeight: 600 }}>
                                                    לקוח: {ticket.created_by_name || "לא צוין"}
                                                </Typography>
                                            </Box>
                                        )}
                                        ...
                                        {isAdmin && (
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                ...
                                                <Typography variant="caption" sx={{ color: '#1976d2', fontWeight: 600 }}>
                                                    סוכן מטפל: {ticket.assigned_to_name || "טרם שובץ"}
                                                </Typography>
                                            </Box>
                                        )}
                                    </Box>
